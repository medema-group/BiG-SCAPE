...
class RecordPairGeneratorGivenNodeSinRef(RecordPairGenerator):
    """Describes a bin of BGCs to generate pairs from, pair generation is limited to given ref to
    singleton ref nodes"""

    def __init__(self, label: str, network: BSNetwork, given_nodes: list[BGCRecord]):
        super().__init__(label)
        self.network: BSNetwork = network
        self.given_nodes: list[BGCRecord] = given_nodes
    ...
    def generate_pairs(self, *args: Any, legacy_sorting=False) -> Iterator[BGCPair]:
        ...
        given_nodes = self.given_nodes

        ref_singletons = self.network.get_singletons(
            node_types=[SOURCE_TYPE.REFERENCE, SOURCE_TYPE.MIBIG]
        )

        for bgc_a in given_nodes:
            for bgc_b in ref_singletons:
                if legacy_sorting:
                    sorted_a, sorted_b = sorted((bgc_a, bgc_b), key=sort_name_key)
                    pair = BGCPair(sorted_a, sorted_b)
    ...
    def num_pairs(self) -> int:
        ...
        num_given_nodes = len(self.given_nodes)
        num_ref_singletons = len(
            self.network.get_singletons(
                node_types=[SOURCE_TYPE.REFERENCE, SOURCE_TYPE.MIBIG]
            )
        ...
        num_pairs = num_given_nodes * num_ref_singletons
